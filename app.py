...
if not st.session_state['logged_in']:
    st.title("🛡️ KUYApaie - Portail de Sécurité")
    ...
    if auth_choice == "Création de Compte KYC":
        with st.form("kyc_form"):
            ...
            if st.form_submit_button("Activer mon profil"):
                ...
                else: st.warning("Vérifiez la complexité de votre mot de passe.")
...
# --- 4. INTERFACE ERP (SI CONNECTÉ) ---
    ...
    st.sidebar.divider()
    st.sidebar.header("⚙️ Paramètres État")
    ...
    tabs = st.tabs(["🛡️ Hedging", "📊 Audit SYSCOHADA", "🇨🇩 Souveraineté", "📜 Rapports BCC", "🏥 Inclusion", "🔄 Réconciliation", "🔐 Cyber", "💸 Paiements", "👨‍✈️ Superviseur"])

    # 1. HEDGING
    with tabs[0]:
        st.header("Protection contre la dépréciation")
        mnt_h = st.number_input("Montant à protéger (USD)", value=1000)
        if st.button("Activer Couverture"):
            log_action("HEDGING", "Contrat Forward activé", mnt_h, "USD")
            st.success("Couverture activée avec succès.")
    ...
    with tabs[2]:
        ...
        mois = st.slider("Durée du placement (Mois)", 3, 24, 12)
        gain = invest * (taux_bon / 100) * (mois / 12)
        st.success(f"Gain estimé : **{gain:,.2f} CDF**. Total à recevoir : **{(invest+gain):,.2f} CDF**")
        if st.button("Souscrire au Bon du Trésor"):
            log_action("SOUVERAINETÉ", f"Placement {mois} mois", invest, "CDF")
    ...
    with tabs[3]:
        ...
        if st.button("Télécharger Rapport BCC/CENAREF"):
            ...
            conn.close()
            st.download_button("Exporter CSV Audit", df_audit.to_csv(index=False), "rapport_bcc.csv")
            st.info("Rapport généré avec horodatage et analyse de risques.")
    ...
    with tabs[7]:
        ...
        if niveau == "BRONZE" and mnt_p > 500:
            st.error("Plafond Bronze (500$) atteint. Veuillez passer au niveau Argent.")
